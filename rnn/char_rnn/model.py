...
class CharRNN:
    ...
    def build_loss(self):
        with tf.name_scope('loss'):
            ...
            loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=y_reshape)
    ...
    def train(self, batch_generator, max_steps, save_path, save_every_n, log_every_n):
        ...
        with self.session as sess:
            ...
            for x, y in batch_generator:
                step += 1
                start = time.time()
                feed = {self.inputs: x, self.targets: y, self.keep_prob: self.train_keep_prob,
                        self.initial_state: new_state}
                batch_loss, new_state, _ = sess.run([self.loss, self.final_state, self.optimizer], feed_dict=feed)
                end = time.time()
                if step % log_every_n == 0:
                    print 'step: {}/{}... loss: {:.4f}... {:.4f} sec/batch'.format(step, max_steps, batch_loss,
                                                                                   (end - start))
